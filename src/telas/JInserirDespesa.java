...
public class JInserirDespesa extends JFrame {
	...
	/**
	 * Launch the application. by: Wanderson R
	 */
	...
	public JInserirDespesa() {
		...
		JFInserirDespesa.setBackground(Color.WHITE);
		...
		data_venc_desp
				.setToolTipText("Insira a data de vencimento da despesa.");
		...
		qtd_parcelas
				.setToolTipText("Insira a quantidade de parcelas que esta despesa possui, caso necess\u00E1rio.");
		...
		valor_despesa
				.setToolTipText("Insira o valor da despesa. N\u00E3o pode conter letras ou valores negativos.");
		...
		Voltar.setIcon(new ImageIcon(JInserirDespesa.class
		...
		lblNewLabel.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				java.util.Date data = data_venc_desp.getDate();// converter o
																// formato da
																// data
				try {
					PreparedStatement pst = conecta.conn
							.prepareStatement("INSERT INTO Despesas (id_perfil, nome_desp, vencimento, descricao_desp, qtd_parcela, valor_desp) VALUES (?,?,?,?,?,?)");// passar
																																										// os
																																										// dados
																																										// por
																																										// parametro
					...
					if (nome_despesa.getText().trim().equals("")) {
						...
						JOptionPane.showMessageDialog(rootPane,
								"O campo NOME DA DESPESA é obrigatório!");
					} else if (data_venc_desp.getDate() == null) {
						JOptionPane.showMessageDialog(rootPane,
								"O campo DATA DE VENCIMENTO é obrigatório!");
					} else if (valor_despesa.getText().replaceAll("[^1-9]", "")
							.isEmpty()) {
		...
		lblNewLabel
				.setToolTipText("Clique aqui para salvar as informa\u00E7\u00F5es.");
		lblNewLabel.setIcon(new ImageIcon(JInserirDespesa.class
		...
		panel.setBorder(new TitledBorder(null,
				"Informa\u00E7\u00F5es de cadastro", TitledBorder.LEADING,
				TitledBorder.TOP, null, null));
